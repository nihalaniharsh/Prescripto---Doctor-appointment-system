...
useEffect(() =>{
  ...
  return docInfo && (
    <div>
      ...
      <div className='sm:mt-6 sm:pl-4 font-medium text-gray-700'>
    <p>Booking slots</p>
    ...
    <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
      {docSlots.length && docSlots[slotIndex].map((item, idx)=>(
       <p onClick={() => setSlotTime(item.time)} className={`text-sm font-light shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'}`} key={idx}>
          {item.time.toLowerCase()}
       </p>
      ))}
    </div>
    <button onClick={bookAppointment} className='my-6 bg-primary font-light rounded-3xl px-12 text-sm text-white py-4 '>Book an appointment</button>
      </div>
